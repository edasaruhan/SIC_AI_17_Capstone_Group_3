...
This checkout is limited to **Sprint 1: Repository Foundation + Data Proof**.
GraphSAGE, transaction-model training, model refinement, final-test evaluation, and
the Streamlit product are outside the active sprint.
...
| Unit/integration tests | PASS | 34 passed in 25.36 seconds |
...
| Model metrics or model comparison | NOT IN SPRINT 1 | No model result is claimed |

The authoritative live checklist is
[`reports/generated/SPRINT_1_STATUS.md`](reports/generated/SPRINT_1_STATUS.md).
...
Sprint 1 does not answer that question. It establishes the validated, leakage-safe
data and feature foundation required to test it in later sprints.
...
flowchart LR
    ...
    G --> Q
    Q -. Sprint 2 only .-> M[Transaction baselines]
```
...
```text
configs/                    versioned quick/full experiment settings
...
src/argus/                  reusable data and feature code
tests/                      automated data, leakage, and feature checks
...
The commands below reproduce the verified Sprint 1 path.
...
contract. Its EDA manifest separately labels exact all-row tables and the
deterministic sampled plot/NetworkX scope. A quick or sampled artifact must never
be described as a full-population result.
...
- Keep identical timestamps together so train/validation/test boundaries remain
  strictly ordered.
- Fit learned transformations on training data only in later modeling work.
- Select models and thresholds on validation data only; keep test labels untouched
  until refinement is frozen.
- Use composite `normalized_bank_id::account_id` node identities.
- Preserve edge direction and repeated transfers.
- Treat PR-AUC, Recall@K, Precision@K, F1, FPR, and alert volume as the future core
  metrics; accuracy is not a primary metric for this imbalanced dataset.
...
pipeline. Full-population NetworkX component/local-subgraph materialization remains
...
- [`ROADMAP.md`](docs/ROADMAP.md): Sprint 1 gates and Sprint 2 recommendations
